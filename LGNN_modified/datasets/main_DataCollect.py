...
def print_configuration(natnet_client):
    ...
    print(
        "    MotiveVersion  %d %d %d %d" % (server_version[0], server_version[1], server_version[2], server_version[3]))
...
if __name__ == "__main__":
    ...
    optionsDict = my_parse_args(sys.argv, optionsDict)  # create a new NatNet client
    ...
    time_prepare = 10  # seconds
    print(f"\nStarting data collection in {time_prepare} seconds ...\n")
    time.sleep(time_prepare)  # time for preparation
    winsound.Beep(frequency=1000, duration=500)

    # Start up the streaming client now that the callbacks are set up.
    # This will run perpetually, and operate on a separate thread.
    is_running = streaming_client.run()  # updates are in __command_thread_function, __process_message
    ...
    # while is_looping:
        ...
        #         print("Showing only received frame numbers and suppressing data descriptions")
        #     elif c1 == 'k':
        #         streaming_client.set_print_level(1)

...
class MarkerData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        for i in range(marker_count):
            ...
            out_str += "%sMarker %3.1d pos : [x=%6.3f,y=%6.3f,z=%6.3f] (unit: mm)\n" % (
            out_tab_str2, i, pos[0] * 1000, pos[1] * 1000, pos[2] * 1000)
        return out_str

    def get_DIY_data(self):
        out_list = []
        out_list_title = []
        cur_model_name = get_as_string(self.model_name)
        if cur_model_name == get_as_string("knob") or cur_model_name == get_as_string("object"):
            marker_count = len(self.marker_pos_list)
            for i in range(marker_count):
                pos = self.marker_pos_list[i]
                for j in range(3):
                    out_list.append("%6.3f" % (pos[j] * 1000))
                out_list_title.append("Marker_%s%d_X" % (cur_model_name, i))
                out_list_title.append("Marker_%s%d_Y" % (cur_model_name, i))
                out_list_title.append("Marker_%s%d_Z" % (cur_model_name, i))
        return out_list, out_list_title
...
class MarkerSetData:
    ...
    def get_DIY_data(self):
        ...
        # Labeled markers
        for marker_data in self.marker_data_list:
            data, title = marker_data.get_DIY_data()
            out_list.extend(data)
            out_list_title.extend(title)
        # Unlabeled markers
        # out_list.extend(self.unlabeled_markers.get_DIY_data())
        return out_list, out_list_title
...
class RigidBodyMarker:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        out_str += "%sPosition: [%6.3f %6.3f %6.3f] (unit: mm)\n" % (
        out_tab_str, self.pos[0] * 1000, self.pos[1] * 1000, self.pos[2] * 1000)
    ...
    def get_DIY_data(self):
        out_list = []
        for i in range(3):
            out_list.append("%6.3f" % (self.pos[i]*1000))
        return out_list
...
class RigidBody:
    ...
    def get_as_string(self, tab_str=0, level=0):
        ...
        out_str += "%s  Position      : [%6.3f, %6.3f, %6.3f] (unit: mm)\n" % (
        out_tab_str, self.pos[0] * 1000, self.pos[1] * 1000, self.pos[2] * 1000)
...
class RigidBodyData:
    ...
    def get_DIY_data(self):
        ...
        for rigid_body in self.rigid_body_list:
            rigid_body.marker_num = rb_num
            out_list.extend(rigid_body.get_DIY_data())
            rb_num += 1
        return out_list
...
class AssetMarkerData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        if (False):
            ...
            out_str += "%sPos      : %6.3f %6.3f %6.3f (unit: mm)\n" % (
            out_tab_str, self.pos[0] * 1000, self.pos[1] * 1000, self.pos[2] * 1000)
            ...
            out_str += " pos : [%6.3f, %6.3f, %6.3f] (unit: mm)" % (
            self.pos[0] * 1000, self.pos[1] * 1000, self.pos[2] * 1000)
...
class LabeledMarkerData:
    ...
    def get_DIY_data(self):
        ...
        for i in range(0, labeled_marker_count):
            labeled_marker = self.labeled_marker_list[i]
            labeled_marker.marker_num = i
            out_list.extend(labeled_marker.get_DIY_data())
        return out_list
...
class MoCapData:
    ...
    def get_DIY_data(self):
        ...
        out_list_marker_data, out_list_marker_title = self.marker_set_data.get_DIY_data()
        out_list_data.extend(out_list_marker_data)
        out_list_title.extend(out_list_marker_title)

        out_list_rigid = self.rigid_body_data.get_DIY_data()
        out_list_data.extend(out_list_rigid)
        for i in range(int(len(out_list_rigid)/3)):
            out_list_title.append("RigidBody" + str(i) + "_X")
            out_list_title.append("RigidBody" + str(i) + "_Y")
            out_list_title.append("RigidBody" + str(i) + "_Z")

        return out_list_data, out_list_title
...
def test_all(run_test=True):
    ...
    if run_test is True:
        test_cases = [
            ["Test Prefix Data 0", "bffba016d02cf2167780df31aee697e1ec746b4c", "generate_prefix_data(0)", True],
            ...
            ["Test Labeled Marker Data 0", "e0dd01035424e8e927a4956c21819a1f0ed18355",
             "generate_labeled_marker_data(0)", True],
            ["Test Force Plate Data 0", "2bb1000049a98b3c4ff8c48c7560af94dcdd32b3", "generate_force_plate_data(0)",
             True],
